...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        pad_zero = Variable(torch.zeros(batch_size, tag_size)).long().to(DEVICE)
        # if self.use_cuda:
        ...
        decode_idx = Variable(torch.LongTensor(seq_len, batch_size)).to(DEVICE)
        # if self.use_cuda:
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        new_tags = Variable(torch.LongTensor(batch_size, seq_len)).to(DEVICE)
        # if self.use_cuda:
